...
from .siren import FourierFeatMapping
    ...
    def __init__(self,
        ...
        if self.use_latent_embedding:
            ...
            if self.mixture_type in ['all-layer', 'layerwise']:
                ...
                if norm_latents:
                    latent_to_mixture = nn.Sequential(nn.Linear(latent_channels, mixture_channels), nn.BatchNorm1d(mixture_channels))
                    latent_to_mixture[0].bias = nn.Parameter(torch.ones(latent_to_mixture[0].bias.size())/ self.k_mixtures)
                else:
                    latent_to_mixture = nn.Linear(latent_channels, mixture_channels)
                    latent_to_mixture.bias = nn.Parameter(torch.ones(latent_to_mixture.bias.size())/ self.k_mixtures)
                self.latent_to_mixture = latent_to_mixture
            elif self.mixture_type in ['two-layers']:
                mixture_channels = self.k_mixtures * self.num_ctx_params
                self.latent_to_mixture = nn.Sequential(nn.Linear(latent_channels, latent_channels), nn.ReLU(), nn.Linear(latent_channels, mixture_channels),nn.BatchNorm1d(mixture_channels))
